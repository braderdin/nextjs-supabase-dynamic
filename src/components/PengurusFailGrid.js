...
export default function PengurusFailGrid({ 
  ...
  return (
    // Mula: Kerangka Bento Asset Manager Vercel Style 2026
    <div className="bg-slate-950/40 backdrop-blur-md border border-slate-900/80 shadow-[2px_2px_0px_0px_rgba(255,255,255,0.01)] font-mono text-xs text-white flex flex-col justify-between transition-all rounded-none">
      ...
      <div>
        ...
        {/* Tamat: Header Toolbar Minimalis */}

        {/* Mula: Layout Tab Mode Paparan */}
        <div className="bg-slate-950/20 border-b border-slate-900/40 p-1.5 px-3 flex items-center justify-end gap-2 text-[10px] select-none text-slate-500">
          ...
          <button
            ...
            className={`px-2 py-0.5 border text-[9px] font-bold uppercase transition-colors rounded-none ${
              modPaparan === 'grid' 
        ...
        {/* Tamat: Layout Tab Mode Paparan */}

        {/* Mula: Borang Dinamik Cipta Item */}
        {modCipta && (
          <form onSubmit={handleCiptaEntiti} className="p-4 bg-slate-950/60 border-b border-slate-900/60 space-y-3 animate-fadeIn">
            <div className="flex items-center gap-2">
              ...
              <input 
                ...
                className="flex-1 bg-slate-900/40 border border-slate-900 px-2 py-1 text-xs text-white focus:outline-none focus:border-slate-800 font-mono rounded-none"
              />
            </div>
            {modCipta === 'fail' && (
              <div className="space-y-1">
                ...
                <textarea 
                  rows={4}
                  placeholder="<!-- Tulis kod HTML/CSS anda di sini -->"
                  value={isiFailBaru}
                  onChange={(e) => setIsiFailBaru(e.target.value)}
                  className="w-full bg-slate-900/40 border border-slate-900 p-2 font-mono text-xs text-slate-200 focus:outline-none focus:border-slate-800 resize-none rounded-none shadow-inner"
                />
              </div>
            )}
        ...
        {/* Tamat: Borang Dinamik Cipta Item */}

        {/* Mula: Grid & List Container Bento Asset */}
        <div className={`p-4 bg-transparent min-h-[240px] max-h-[380px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-900 scrollbar-track-transparent ${
          modPaparan === 'grid' 
          ...
          {loadingFail && (
            <div className="col-span-full text-center text-slate-600 font-mono py-12 animate-pulse select-none">
              [ Membaca gelombang isyarat storan Cloudflare R2... ]
            </div>
          )}
          ...
          {!loadingFail && itemDipapar.length === 0 && (
            <div className="col-span-full text-center text-slate-600 font-mono text-[11px] py-12 select-none">
              [ Direktori kosong. Sila klik "+ Fail" untuk memulakan projek teratak abangku! ]
            </div>
          )}
          ...
          {!loadingFail && itemDipapar.map((item, indeks) => {
            ...
            // Mula: Suntikan Ikon Dawai Wireframe SVG 2026 Minimalis
            ...
            // Tamat: Suntikan Ikon Dawai Wireframe SVG 2026 Minimalis

            // Mula: Sub-Layout Paparan Gaya Senarai (List View)
            if (modPaparan === 'list') {
              return (
                <div 
                  key={indeks}
                  className="w-full flex items-center justify-between bg-slate-900/30 border border-slate-900 hover:border-slate-800 p-2 px-3 group transition-all select-none rounded-none"
                >
                  <div 
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button 
                      ...
                      className="bg-slate-950/60 border border-slate-900 text-red-500/70 hover:text-red-400 text-[9px] px-2 py-0.5 font-bold uppercase transition-colors rounded-none"
                    >
                      Padam
                    </button>
                  </div>
            ...
            // Tamat: Sub-Layout Paparan Gaya Senarai (List View)

            // Mula: Sub-Layout Paparan Gaya Grid Bento (Grid View)
            return (
              <div 
                key={indeks}
                className="bg-slate-900/20 border border-slate-900/80 hover:border-slate-800 p-3.5 flex flex-col items-center justify-between text-center relative group transition-all select-none min-h-[105px] rounded-none shadow-sm"
              >
                <div className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 flex gap-1 transition-opacity z-10">
                  <button 
                    type="button"
                    onClick={() => handlePadamEntiti(item.laluanFull)}
                    className="bg-slate-950 border border-slate-900 text-red-500/60 hover:text-red-400 text-[8px] px-1.5 rounded-none font-bold py-0.5 uppercase transition-all"
                    title="Padam Fail"
                  >
                    ×
                  </button>
                ...
                <div 
                  onClick={() => adakahFolder ? setFolderSemasa(item.laluanFull) : onFileSelect(item)}
                  className="w-full flex flex-col items-center cursor-pointer py-1 flex-1 justify-center"
                >
                  <span className="mb-2 scale-110 block transition-transform duration-300 group-hover:scale-120">{lambangIkon}</span>
                  <span className="text-[11px] font-medium text-slate-400 group-hover:text-white truncate w-full px-1">
                    {item.nama}
                  </span>
                </div>
            ...
            // Tamat: Sub-Layout Paparan Gaya Grid Bento (Grid View)
          })}
        </div>
        {/* Tamat: Grid & List Container Bento Asset */}
      </div>

    </div>
    // Tamat: Kerangka Bento Asset Manager Vercel Style 2026
  );
}